...
$AzKeyVaultCertificateSecret = Get-AzKeyVaultSecret -VaultName $keyvaultName -Name $RunAsAccountName -AsPlainText

# Convert the certificate secret to a base64
$AzKeyVaultCertificateSecretBytes = [System.Convert]::FromBase64String($AzKeyVaultCertificateSecret)
...
If (!(Get-AzADApplication -DisplayName $RunAsAccountName)){
    ...
    New-AzADAppCredential -ApplicationId $AzADApplicationRegistration.ApplicationId `
